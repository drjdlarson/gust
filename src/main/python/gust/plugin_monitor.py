...
class PluginMonitor(QObject):
    ...
    def get_plugin_exe(self, plugin_name):
        return os.path.join(
            self.plugin_dir, plugin_name, "{}{}".format(plugin_name, self.file_ext)
        )
    ...
    def _proc_pending_data(self):
        ...
        while self.udp_sock.hasPendingDatagrams():
            datagram = self.udp_sock.receiveDatagram(
                self.udp_sock.pendingDatagramSize()
            )
            ...
            logger.info(
                "Rec from Plugin: {:s} ID: {:d} Data: {:}".format(p_name, p_id, data)
            )
    ...
    def _start_plugins(self):
        ...
        for p_name in des_plugins:
            ...
            for p_id in plugin_ids:
                ...
                args = ["-p {:d}".format(self.port), "{:d}".format(p_id)]
